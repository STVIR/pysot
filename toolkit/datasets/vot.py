...
class VOTVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect,
        ...
        img_names = [os.path.join(root, video_dir, img_name) for img_name in img_names]
...
class VOTLTVideo(Video):
    ...
    Args:
        ...
        gt_rect: groundtruth rectangle
    """
    def __init(self, name, root, video_dir, init_rect, img_names, gt_rect, load_img=False):
        img_names = [os.path.join(root, video_dir, img_name) for img_name in img_names]
        ...
        if not load_img:
            img_name = os.path.join(root, img_names[0])
            img = np.array(Image.open(img_name), np.uint8)
            self.width = img.shape[1]
            self.height = img.shape[0]
        self.confidence = {}
