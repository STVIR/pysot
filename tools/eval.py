...
from pysot.toolkit.datasets import (
    LaSOTDataset,
    NFSDataset,
    OTBDataset,
    UAVDataset,
...
from pysot.toolkit.evaluation import (
...
parser.add_argument(
    "--show_video_level", "-s", dest="show_video_level", action="store_true"
)
...
def main():
    ...
    trackers = glob(
        os.path.join(args.tracker_path, args.dataset, args.tracker_prefix + "*")
    )
    ...
    if "OTB" in args.dataset:
        ...
        benchmark.show_result(
            success_ret, precision_ret, show_video_level=args.show_video_level
        )
        ...
        with Pool(processes=args.num) as pool:
            for ret in tqdm(
                pool.imap_unordered(ar_benchmark.eval, trackers),
                desc="eval ar",
                total=len(trackers),
                ncols=100,
            ):
