...
def run_tracker(tracker, img, gt, video_name, restart=True):
    ...
    if restart:  # VOT2016 and VOT 2018
        ...
        toc /= cv2.getTickFrequency()
        print(
            "Video: {:12s} Time: {:5.1f}s Speed: {:3.1f}fps".format(
                    ...
                    for ins in args.search_region:
                        ...
                        if "VOT2016" == args.dataset or "VOT2018" == args.dataset:
                            video_path = os.path.join(
                                tracker_path, "baseline", video.name
                            )
                            result_path = os.path.join(
                                video_path, video.name + "_001.txt"
                            )
                            if _check_and_occupation(video_path, result_path):
                                continue
                            pred_bboxes = run_tracker(
                                tracker,
                                video.imgs,
                                video.gt_traj,
                                video.name,
                            ...
                            video_path = os.path.join(
                                tracker_path, "longterm", video.name
                            )
                            result_path = os.path.join(
                            ...
                            video_path = os.path.join(
                                "epoch_result", tracker_path, video.name
                            )
                            if not os.path.isdir(video_path):
                                os.makedirs(video_path)
                            result_path = os.path.join(
