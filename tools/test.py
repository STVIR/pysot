...
from pysot.toolkit.utils.region import vot_float2str, vot_overlap
...
parser.add_argument(
    "--snapshot", default="", type=str, help="snapshot of models to eval"
)
...
def main():
    ...
    if args.dataset in ["VOT2016", "VOT2018", "VOT2019"]:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if args.vis and idx > frame_counter:
                    cv2.polylines(
                        img,
                        [np.array(gt_bbox, np.int).reshape((-1, 1, 2))],
                        True,
                        (0, 255, 0),
                        3,
                    )
                    if cfg.MASK.MASK:
                        cv2.polylines(
                            img,
                            [np.array(pred_bbox, np.int).reshape((-1, 1, 2))],
                            True,
                            (0, 255, 255),
                            3,
                        )
            ...
            video_path = os.path.join(
                "results", args.dataset, model_name, "baseline", video.name
            )
            ...
            print(
                "({:3d}) Video: {:12s} Time: {:4.1f}s Speed: {:3.1f}fps Lost: {:d}".format(
        ...
        for v_idx, video in enumerate(dataset):
            ...
            if "VOT2018-LT" == args.dataset:
                video_path = os.path.join(
                    "results", args.dataset, model_name, "longterm", video.name
                )
                if not os.path.isdir(video_path):
                    os.makedirs(video_path)
                result_path = os.path.join(video_path, "{}_001.txt".format(video.name))
